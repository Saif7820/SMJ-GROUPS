...
import img101 from '../../assets/images/101.png';
import img102 from '../../assets/images/102.png';
import img103 from '../../assets/images/103.png';
import img104 from '../../assets/images/104.png';
import img105 from '../../assets/images/105.png';
import img106 from '../../assets/images/106.png';
import img107 from '../../assets/images/107.png';

const constructionImages = [
  img101, img102, img103, img104, img105, img106, img107
];
...
const Construction = () => {
  ...
  const nextImage = () => {
    setPhotoIndex((prev) => (prev + 1) % constructionImages.length);
  };

  const prevImage = () => {
    setPhotoIndex((prev) => (prev + constructionImages.length - 1) % constructionImages.length);
  };
    ...
    <>
      <section className="bg-white py-12 md:py-24 overflow-hidden">
        <div className="max-w-[1700px] mx-auto px-4 sm:px-6 md:px-28">
          ...
          {/* Slider Container */}
          <div className="relative group/construction-gallery px-1 md:px-0">
            ...
            {/* Prev Button */}
            <button className="construction-prev absolute -left-3 md:-left-6 top-1/2 -translate-y-1/2 z-20 w-11 h-11 flex items-center justify-center bg-white shadow-md rounded-full border border-gray-100 transition-all duration-300 cursor-pointer opacity-0 pointer-events-none group-hover/construction-gallery:opacity-100 group-hover/construction-gallery:pointer-events-auto max-sm:hidden">
              <svg
                xmlns="http://w3.org"
                fill="none"
                viewBox="0 0 24 24"
            ...
            {/* Next Button */}
            <button className="construction-next absolute -right-3 md:-right-6 top-1/2 -translate-y-1/2 z-20 w-11 h-11 flex items-center justify-center bg-white shadow-md rounded-full border border-gray-100 transition-all duration-300 cursor-pointer opacity-0 pointer-events-none group-hover/construction-gallery:opacity-100 group-hover/construction-gallery:pointer-events-auto max-sm:hidden">
              <svg
                xmlns="http://w3.org"
                fill="none"
                viewBox="0 0 24 24"
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 z-[150] bg-black/95 flex items-center justify-center p-4">
          ...
          <div className="absolute top-6 left-6 z-[160] text-white text-lg font-medium tracking-wide">
            {photoIndex + 1} / {constructionImages.length}
          </div>
          ...
          {/* Image */}
          <img
            src={constructionImages[photoIndex]}
            alt={`Construction Update full ${photoIndex + 1}`}
